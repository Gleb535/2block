    ...
    for (int i = 0; i < w; i++) // Первый цикл для перебора столбцов
    {
        ...
        for (int j = i; j < h - 1; j++) // Второй цикл для перебора строк
        {
            ...
            for (int k = 0; k < w; k++) // Вычитаем из каждого элемента строки верхнее значение домноженное на r
            {
                ...
                printf("\n");
                for (int i = 0; i < h; i++) // Выводим изменения массива
                {
                    for (int j = 0; j < w; j++)
                        printf("%.1f ", temp[i][j]);
                    printf("\n");
...
int zero_str_stlb(double **ar, int n, int k) // поиск нулевых строк, столбцов
{
    int z[3] = {0, 0, 0};
...
{
    if ((h != w) || (zero_str_stlb(ar, h, w) == -1)) // возможно ли посчитать определитель
    {
        printf("NO!\n");
        return 0;
    }
